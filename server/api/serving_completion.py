...
TypeCreateLogProbsFn = Callable[[TypeTokenIDs, TypeTopLogProbs, Optional[int], int], LogProbs]
...
async def completion_stream_generator(
    ...
    raw_request: Request,
    on_abort,
    result_generator: AsyncIterator[Tuple[int, PipelineOutput]],
    ...
    first_scheduled_time = [float("inf")] * num_prompts
    first_token_time = [float("inf")] * num_prompts
    async for prompt_idx, res in result_generator:
        res: PipelineOutput
        first_scheduled_time[prompt_idx] = res.first_scheduled_time
        first_token_time[prompt_idx] = res.first_token_time
        raw_request.state.first_scheduled_time = min(first_scheduled_time)
        raw_request.state.first_token_time = min(first_token_time)
...
def request_output_to_completion_response(
    final_res_batch: List[PipelineOutput],
    request: CompletionRequest,
    create_logprobs_fn: TypeCreateLogProbsFn,
    request_id: str,
    created_time: int,
...
class OpenAIServingCompletion(OpenAIServing):
    async def create_completion(self, request: CompletionRequest, raw_request: Request):
        ...
        if request.suffix is not None:
            raise ValueError("suffix is not currently supported")
        if request.logit_bias is not None and len(request.logit_bias) > 0:
            raise ValueError("logit_bias is not currently supported")
        ...
        for i, prompt in enumerate(prompts):
            ...
            generators.append(self.pipeline.stream(input_ids, pipeline_params, f"{request_id}-{i}"))
        ...
        if stream:
            generator = completion_stream_generator(
                request,
                raw_request,
                self.pipeline.abort_stream_request,
                result_generator,
        ...
        final_res_batch: List[PipelineOutput] = [None] * len(prompts)
        async for i, res in result_generator:
            if await raw_request.is_disconnected():
                # Abort the request if the client disconnects.
                await self.pipeline.abort_stream_request(f"{request_id}-{i}")
                raise asyncio.CancelledError("Client disconnected")
            # 这里手动把 prompt 加上，因为 pipeline 调用 stream 的时候用的是 token ids，prompt 是 None。
            res.prompt = prompts[i]
            final_res_batch[i] = res
        response = request_output_to_completion_response(
            final_res_batch, request, self._create_logprobs, request_id, created_time, model_name
        )
        raw_request.state.generated_text = [c.text for c in response.choices]
