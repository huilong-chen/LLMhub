...
# 根据当前的上下文并行等级（context parallel rank）来选择和调整位置嵌入（position embedding）。
...
# RotaryEmbedding类实现了RoPE机制，允许语言模型更好地捕捉位置依赖关系。
# 其初始化函数定义了相关参数，如kv_channels、rotary_percent、rotary_interleaved等。
#
# 在forward方法中，根据输入的最大序列长度和偏移量生成频率矩阵，并对其进行调整。
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, max_seq_len: int, offset: int = 0) -> Tensor:
        ...
        seq = (
            ...
            + offset
        ) # position ids from 0 to max_seq_len

        if self.seq_len_interpolation_factor is not None:
            seq *= 1 / self.seq_len_interpolation_factor

        print(seq.shape) # torch.Size([8])
        print(self.inv_freq.shape) # torch.Size([2])
        freqs = torch.outer(seq, self.inv_freq) # 计算外积
        print(freqs)
        ...
        #         [5.0000, 0.0500],
        #         [6.0000, 0.0600],
        #         [7.0000, 0.0700]], device='cuda:0')

        # first part even vector components, second part odd vector components, （"第一部分偶数向量分量，第二部分奇数向量分量"）
        #  2 * dim in dimension size
        if not self.rotary_interleaved: # if True
        ...
        #         [4.0000, 0.0400, 4.0000, 0.0400],
        #         [5.0000, 0.0500, 5.0000, 0.0500],
        #         [6.0000, 0.0600, 6.0000, 0.0600],
        #         [7.0000, 0.0700, 7.0000, 0.0700]], device='cuda:0')
        print(emb.shape)
        # torch.Size([8, 4])
...
# 这个函数将RoPE嵌入应用于输入张量t。它首先计算余弦和正弦分量，然后将它们与输入张量进行组合，返回应用RoPE后的张量。
...
# 这个函数将RoPE嵌入应用于thd格式的输入张量t，thd格式通常用于处理分批的序列数据。
...
# 该函数根据配置选择合适的RoPE应用方法，如果启用了融合计算且可用，则使用Apex提供的优化版本，否则使用常规的方法。
