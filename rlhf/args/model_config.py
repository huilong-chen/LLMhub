...
class MyModelConfig(ModelConfig):
    model_name_or_path: Optional[str] = field(default="model/Qwen2-7B", metadata={"help": "模型加载路径"})
    ...
    torch_dtype: Optional[str] = field(default='bfloat16', metadata={"help": "4位精度计算的数据类型",
    ...
    lora_task_type: str = field(default="SEQ_CLS",
                                metadata={"help": "The task_type to pass for LoRA (use SEQ_CLS for reward modeling)"})
    ...
    bnb_4bit_quant_type: Optional[str] = field(default="nf4", metadata={
        "help": "4位精度量化的类型。这里设置为nf4 表示使用nf4量化类型。"})
    use_bnb_nested_quant: bool = field(default=True,
                                       metadata={"help": "是否使用双精度量化。如果设置为True，则使用双精度量化"})
